...
public class Ej10 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        {
            if (horaIngresada >= HORA_APERTURA && horaIngresada < HORA_CIERE)
                estaAbierto = true;
            else
                estaAbierto = false;

            if (estaAbierto == true)
                Debug.Log("El establecimiento está abierto");
            else
                Debug.Log("El establecimiento está cerrado");
        }
    }
